...
We take advantage of this localized change in our energy function as well. Calculating the product $\mathbf{x}^TG \mathbf{x}$ from scratch to get $\mathcal{L}$ at every single step is computationally expensive and bogs everything down. Since we are only modifying two components of the $\lambda$ vector, we need not to redo all our math.
...
Now with the double summation, we want the addends that contain an \(\lambda _i\) or (inclusive) \(\lambda_j\). We'll do it first with the terms that only contain one modified lambda.
\begin{align*}
