...
Instead, we made the algorithm about 20 times faster by picking just two random indices, $i$ and $j$. We apply a random increment to $\lambda_i$ and simply adjust $\lambda_j$ to balance it out and satisfy the constraint. We take advantage of this localized change in our energy function as well. Calculating the product $\mathbf{x}^TG \mathbf{x}$ from scratch to get $\mathcal{L}$ at every single step is computationally expensive and bogs everything down. Since we are only modifying two components of the $\lambda$ vector, let's not redo all the math. We can keep the last computed energy and just add or subtract the change:
$$
