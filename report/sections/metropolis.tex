...
Metropolis assumes CDRs are located in the same position for all antibodies being analyzed; in order for this to be valid, sequences
need to be properly aligned beforehand.
...
\subsection{Simulated Annealing Schedule (\texorpdfstring{$\beta$}{beta})}
...
The total energy, $E_\text{total}$, is a weighted sum of three components, which we aim to minimize:
    ...
    \begin{enumerate}
        ...
        \item \textbf{Wandering Penalty Energy, }$\mathbf{E_\text{penalty}}$:
        ...
        $P$ is the mutation penalty (\texttt{AA\_MUTATION\_PENALTY}), and $\delta$ is the ``generalized Kronecker delta'' (logical if gate). 
...
\hline
\textbf{Parameter} & \textbf{Value\futnotetext} & \textbf{Description} \\
\hline
...
\texttt{w\_log} & 0.4 (\texttt{head.h}) & Weight for the log-humanness energy component. \\
\texttt{w\_prop} & 0.4 (\texttt{head.h}) & Weight for the property-distance energy component. \\
\texttt{w\_penalty} & 0.2 (\texttt{head.h}) & Weight for the wandering penalty energy component. \\
...
\texttt{AA\_MUTATION\_PENALTY} {($P$)} & 1.5 (\texttt{head.h}) & Constant penalty added for any AA mutation away from the original. \\
\hline
\end{tabular}
\label{tab:metropolis_params}
\end{table}
\footnotetext{Provisional values.}
\newpage